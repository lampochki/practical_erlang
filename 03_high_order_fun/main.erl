...
-export([sample_champ/0, get_stat/1, filter_sick_players/1, make_pairs/2, get_team_stat/1, filter_sick/1, make_pairs_test/0]).
...
get_stat(Champ) ->
  ...
  {TotalTeams, TotalPlayers, TotalAge, TotalRating} = lists:foldl(fun({AvgPlayers, AvgAge, AvgRating}, {TotalTeams, TotalPlayers, TotalAge, TotalRating}) ->
    {TotalTeams + 1, TotalPlayers + AvgPlayers, TotalAge + AvgAge, TotalRating + AvgRating}
      end, {0,0,0,0}, TeamStat),
  {TotalTeams, TotalPlayers, TotalAge/TotalPlayers, TotalRating/TotalPlayers}.

get_team_stat({team, _, Players}) ->
  F = fun({player, _, Age, Rating,_}, {AvgPlayers, AvgAge, AvgRating}) ->
    {AvgPlayers + 1, AvgAge + Age, AvgRating + Rating}
      end,
...
filter_sick_players(Champ) ->
  HealthyTeams = lists:map(fun filter_sick/1, Champ),
