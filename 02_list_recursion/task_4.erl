...
-export([dropwhile/2, takewhile/2, dropwhile_test/0, takewhile_test/0]).
...
dropwhile(Pred,[],Acc) -> Acc;
dropwhile(Pred, [First | Rest], Acc) ->
  case Pred(First) of
    false -> dropwhile(Pred, [], [First|Rest]);
    true -> dropwhile(Pred, Rest, Acc)
  end.
...
takewhile(Pred, [], Acc) -> lists:reverse(Acc);
takewhile(Pred, [First | Rest], Acc) ->
  case Pred(First) of
    true -> takewhile(Pred, Rest, [First | Acc]);
